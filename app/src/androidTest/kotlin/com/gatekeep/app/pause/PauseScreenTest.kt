...
class PauseScreenTest {
    ...
    fun pa01_profilePause_fromUi() {
        composeRule.onNodeWithTag(GatekeepTestTags.PAUSE_FIVE_MIN).performClick()
        composeRule.waitForIdle()
    }

    @Test
    fun pa04_globalPause_fromUi() {
        composeRule.onNodeWithText("All", substring = true, ignoreCase = true).performClick()
        composeRule.waitForIdle()
        composeRule.onNodeWithText("15 min", substring = true).performClick()
    }
